...
def _cover_block(src, alt, class_name, dim_ratio, inner_markup, qa_flag, extra_class=""):
    """A native core/cover exactly as the block editor's own `save`
    serializes it -- verified against wp.blocks.getBlockContent() on the
    dev site. The <img> comes *before* the <span> overlay (reversing them
    fails block validation: "Expected tag name `img`, instead saw `span`"),
    and the overlay's dim class is `has-background-dim-N` only when N is
    not the 50 default, always followed by `has-background-dim`.
    """
    # Matches core's dimRatioToClass(): only an *exact* 50 (the control's
    # default) collapses to the bare class. Every other ratio -- 0
    # included -- gets a numbered class rounded to the nearest 10, e.g.
    # dimRatio:0 still renders "has-background-dim-0 has-background-dim",
    # and a captured 24% gets "has-background-dim-20 has-background-dim".
    # Verified both against core/cover's own save() on the dev site.
    if dim_ratio == 50:
        dim_classes = "has-background-dim"
    else:
        rounded = round(dim_ratio / 10) * 10
        dim_classes = f"has-background-dim-{rounded} has-background-dim"
    # `alt` MUST be in the block's JSON attributes, not just the <img> tag:
    # core/cover's save() renders the <img alt> from the attribute, so an
    # alt in markup but absent from JSON fails block validation
    # ("Content generated by save function" has alt="", markup has text).
    alt_attr = f',"alt":"{alt}"' if alt else ""
    full_class = f"{class_name} {extra_class}".strip()
    return (
        '<!-- wp:cover {"url":"' + src + '"' + alt_attr + ',"dimRatio":' + str(dim_ratio) + ','
        '"overlayColor":"primary","align":"full","className":"' + full_class + '"} -->\n'
        f'<div class="wp-block-cover alignfull {full_class}">'
        f'<img class="wp-block-cover__image-background" alt="{alt}" src="{src}" data-object-fit="cover"/>'
        '<span aria-hidden="true" class="wp-block-cover__background '
        f'has-primary-background-color {dim_classes}"></span>'
...
def block_to_gutenberg(block):
    ...
    if t == "hero":
        ...
        # A translucent panel behind the text cluster -- see
        # crawler_agent.py's extract_hero(). Confirmed on this site's home
        # page: without it, the heading/sub-tagline sit directly on the
        # (now largely undimmed, see dim_ratio below) photo and either
        # need a heavy dark overlay to stay legible -- which is what made
        # the hero image look muddy compared to live -- or lose contrast
        # entirely. .migration-hero-boxed (see _extra_css_rules()) also
        # cancels the plain ".migration-hero" white-text override, since
        # the box's own light background needs the theme's normal (dark)
        # heading color, not white-on-white.
        boxed = bool(block.get("heading_box"))
        if boxed:
            inner_markup = (
                '<!-- wp:group {"className":"migration-hero-box","layout":{"type":"constrained"}} -->\n'
                '<div class="wp-block-group migration-hero-box">\n'
                f'{inner_markup}\n'
                '</div>\n'
                '<!-- /wp:group -->'
            )
        ...
        # dim_ratio is the actual overlay strength captured from the live
        # page (0 when, as on this site's home hero, there's no dark
        # overlay at all -- readability comes from the white box instead)
        # rather than a hard-coded guess.
        dim_ratio = block.get("dim_ratio", 0)
        return _cover_block(
            src, alt, "migration-hero", dim_ratio, inner_markup,
            "hero background image still points at the original site "
            ...
            extra_class="migration-hero-boxed" if boxed else "",
        )
    ...
    if t == "media_text":
        ...
        # GoDaddy alternates which side the image sits on per section
        # (confirmed on /services and /about: flex-direction:row-reverse
        # on roughly every other pair) and, on one section per site so
        # far (About's founder photo), uses a narrower image column than
        # the usual 50/50 -- see crawler_agent.py's mark_media_text_pairs().
        # A plain "always image-left, always 50/50" render made every
        # alternating section look wrong and stretched a modest headshot
        # to fill half the row. Markup verified against
        # wp.blocks.getBlockContent() on the dev site: mediaPosition
        # "right" reorders the two DOM children (content first, then
        # figure) and swaps which side of the grid-template-columns style
        # carries mediaWidth.
        right = block.get("image_side") == "right"
        width_pct = block.get("image_width_pct")
        attrs = ['"align":"wide"', '"mediaType":"image"']
        classes = "wp-block-media-text alignwide is-stacked-on-mobile"
        style_attr = ""
        if right:
            attrs.append('"mediaPosition":"right"')
            classes += " has-media-on-the-right"
        if width_pct and abs(width_pct - 50) >= 8:
            attrs.append(f'"mediaWidth":{width_pct}')
            cols = f"auto {width_pct}%" if right else f"{width_pct}% auto"
            style_attr = f' style="grid-template-columns:{cols}"'
        figure = f'<figure class="wp-block-media-text__media"><img src="{src}" alt="{alt}"/></figure>'
        content_div = f'<div class="wp-block-media-text__content">\n{content_blocks}\n</div>'
        children = f"{content_div}{figure}" if right else f"{figure}{content_div}"
        return (
            '<!-- wp:media-text {' + ",".join(attrs) + '} -->\n'
            f'<div class="{classes}"{style_attr}>{children}</div>\n'
            '<!-- /wp:media-text -->\n'
