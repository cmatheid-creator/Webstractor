...
Generated: 2026-09-06 02:53 UTC
...
- `repair-migration.zip` — the **Stratecon Migration Repair** plugin. After importing and publishing the pages, upload **`repair-migration.zip`** via Plugins → Add New → Upload Plugin and click Activate — it runs once, shows a report, then deactivates itself. Repoints broken re-hosted image URLs at the file WordPress actually saved, pulls media-library copies of the stock images the importer couldn't, and sets the static front page. No server/shell access needed; safe to activate again. (A shell, where available, can instead run `php wp-content/plugins/repair-migration/repair-migration.php` directly.)
