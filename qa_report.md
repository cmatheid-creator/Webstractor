...
Generated: 2026-08-28 00:28 UTC
...
- **Navigation menu** (20 item(s), matching the site's real nav structure including page hierarchy) included as a WordPress menu named "Migrated Site Menu", ready on import. If the target site already has a menu with the same name (e.g. from theme demo content), WordPress merges into it rather than creating a separate one — check for and remove any unrelated/invalid items after import. **One manual step required regardless**: WordPress doesn't auto-assign an imported menu to a theme location — go to Appearance → Menus (or the Site Editor's Navigation block for a block theme) and assign "Migrated Site Menu" to your primary menu location.
