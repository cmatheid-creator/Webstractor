...
Generated: 2026-08-28 03:40 UTC
...
- **Navigation menu** (20 item(s), matching the site's real nav structure including page hierarchy) is included **twice**, in two different WordPress formats, so it works automatically regardless of which kind of theme the target site uses:
  ...
  - A block-theme navigation entry (`wp_navigation`, also named "Migrated Site Menu") for block themes like Twenty Twenty-Four. Add a Navigation block anywhere in the Site Editor and use its **"Choose or create a Navigation menu"** picker to select "Migrated Site Menu" — no menu-location registration needed, since block themes don't use those. This is the one manual click left in the process; everything else in the menu itself (structure, labels, links, hierarchy) is generated. If the target site already has a menu/navigation with the same name (e.g. from theme demo content), WordPress merges into it rather than creating a separate one — check for and remove any unrelated/invalid items after import.
